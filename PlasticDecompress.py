...
original_image=[]
recon_im=[]
for filename in glob.glob("C:\\Users\\prana\\Desktop\\Reinforcement learning\\BTP\\Assignmnt 2\\train\\train\\*.png"):
...
np.save('Original',original_image)
np.save('Recon',recon_im)
original_image=np.load('Original.npy')
recon_im=np.load('Recon.npy')
...
tensor_x = torch.stack([torch.Tensor(i) for i in recon_im])#i/p to model
tensor_y = torch.stack([torch.Tensor(i) for i in original_image])#y the predicted 
my_dataset = utils.TensorDataset(tensor_x,tensor_y) # create your datset

loader_train=DataLoader(dataset=my_dataset,batch_size=5, shuffle=True)


model=DeoisingCNN(num_channels=64,num_of_layers=6).cuda()
import pytorch_ssim
criterion = nn.MSELoss()
#criterion = pytorch_ssim.SSIM(window_size = 9).cuda()
from torch import optim
optimizer = optim.Adam(model.parameters(), lr=0.0001,weight_decay=0.0001)
#optimizer = optim.SGD(model.parameters(), lr=0.0001, momentum=0.95,weight_decay=0.01,dampening=1e-4)
...
for epoch in range(epochs):
    for i, (im1,tg1) in enumerate(loader_train):
        images = Variable(im1.cuda()/255.0)
        
        images=images.unsqueeze(1)
        target = Variable(tg1.cuda()/255.0)
        ...
        if(epoch==1 and i==0):
            ...
            flag=False
        if(flag):
            loss=criterion(outputs+images, target)
        if(not(flag)):
            criterion = pytorch_ssim.SSIM(window_size = 9).cuda()
            loss=1-criterion(outputs+images, target)
        '''
        loss=criterion(outputs+images, target)
        criterion = pytorch_ssim.SSIM(window_size = 9).cuda()
        loss=loss+0.05*(1-criterion(outputs+images, target))
        
        '''
        loss.backward()
        running_loss=running_loss+loss
        optimizer.step()
        #running_ssim=ssim(images[0,0,:,:].cpu().data.numpy()+outputs[0,0,:,:].cpu().data.numpy(),target[0,0,:,:].cpu().data.numpy())+running_ssim
        running_ssim=ssim(outputs[0,0,:,:].cpu().data.numpy()+images[0,0,:,:].cpu().data.numpy(),target[0,0,:,:].cpu().data.numpy())+running_ssim
        
        if iter % 10 == 0:
            print('Epoch:{}. Iteration: {}. Loss: {}. SSIM: {}'.format(epoch,iter, running_loss/10,running_ssim/10))
            l.append(psnr(outputs+images,target).cpu().item())
            itr.append(iter)
            plt.plot(itr,l)
            running_loss=0
            running_ssim=0
...
cv2.imshow('re Image',(images.cpu().data.numpy()[0,0,:,:]*255).astype(np.uint8))

cv2.imshow('Deblurred Image',(im.cpu().data.numpy()[0,0,:,:]*255).astype(np.uint8))
cv2.imshow('Original Image',(target.cpu().data.numpy()[0,0,:,:]*255).astype(np.uint8))
   
cv2.waitKey(50000)
cv2.destroyAllWindows()

cv2.imwrite('Recon.png',(images.cpu().data.numpy()[0,0,:,:]*255).astype(np.uint8))
cv2.imwrite('RE-Model.png',(im.cpu().data.numpy()[0,0,:,:]*255).astype(np.uint8))
cv2.imwrite('Ori Image.png',(target.cpu().data.numpy()[0,0,:,:]*255).astype(np.uint8))
'''
model.eval()
est=model(images)
test=target[1:2,:,:,:]
cv2.imwrite('input5.png',(images.cpu().data.numpy()[4,0,:,:]*255).astype(np.uint8))
cv2.imwrite('Restored5.png',(images.cpu().data.numpy()[4,0,:,:]*255+est.cpu().data.numpy()[4,0,:,:]*255).astype(np.uint8))
cv2.imwrite('Target5.png',(target.cpu().data.numpy()[4,0,:,:]*255).astype(np.uint8))
